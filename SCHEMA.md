...
让一个目标派生出多次"提前"提醒。格式 `"T-<n>d"`（提前 n 天）或 `"T-<n>h"`（提前 n 小时）。

```jsonc
"NotifyOffset": ["T-3d", "T-1d"]   // 目标前 3 天、前 1 天 各提醒一次
```

- **不设 NotifyOffset**：在目标时刻当天/当时触发一次（默认行为）。
- **设了 NotifyOffset**：只在 `目标 + 各偏移` 触发；⚠️ **此时不再自动包含"目标当天"**。

> ⚠️ **已知缺口（待修）**：`parseOffset` 目前只认 `d`/`h` 后缀，**不支持 `"T-0"` / `"T"`（当天，偏移 0）**——写了会解析失败。因此当前**无法表达"提前 N 天 + 当天都提醒"**。修复方向：`parseOffset` 把 `"T"` / `"T-0"` 解析为 `0`。
